...
The link points at `https://localsme.github.io/<slug>/` — one `blog/` short of
the real URL, `https://localsme.github.io/blog/<slug>/`.
...
curl -s https://localsme.github.io/sitemap-0.xml   | grep -oE '<loc>[^<]*</loc>' | sed -E 's#</?loc>##g; s#https://localsme.github.io##'   | sort > /tmp/live.txt
...
curl -s -H "Authorization: Bearer $T"   https://api.github.com/repos/LocalSME/LocalSME.github.io   | node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const j=JSON.parse(s);console.log(j.message?'FAIL: '+j.message:'permissions: '+JSON.stringify(j.permissions))})"
unset T
```
...
```bash
gh api repos/LocalSME/LocalSME.github.io/contents/src/content/blog/<slug>.md --jq '.content' | base64 -d | head
```
